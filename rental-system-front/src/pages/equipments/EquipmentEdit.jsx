...
import { getEquipment, updateEquipment } from "./api/EquipmentsApi";
...
export default function EquipmentEdit() {
  ...
  const submit = (ev) => {
    ...
    updateEquipment(id, payload)
      ...
      .catch((err) => {
        alert(extractApiError(err, 'Erro ao atualizar o equipamento. Tente novamente.'));
      });
  };

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }} className="p-6">
      <Breadcrumbs items={[{ label: "Equipamentos", to: "/equipments" }, { label: "Editar" }]} />

      <h1 className="text-2xl font-semibold mb-4">Editar Equipamento</h1>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="flex justify-end">
          <button type="submit" className="px-4 py-2 bg-gray-900 text-white rounded text-sm hover:bg-gray-800 transition">Salvar</button>
        </div>
      </form>
    </motion.div>
  );
